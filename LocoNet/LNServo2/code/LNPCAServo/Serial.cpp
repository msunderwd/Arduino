...
extern void printServoInfo(int servo);
...
void handleSerialInput() {
  ...
  if (cmd.equals("LN")) {
    ...
    int servo = Serial.readStringUntil(' ').toInt();
    String modeval = Serial.readStringUntil(' ');
    ServoMode mode = SERVO_MODE;
    if (modeval.equals("SERVO")) { mode = SERVO_MODE; }
    else if (modeval.equals("ONOFF")) { mode = ONOFF_MODE; }
    else { mode = SERVO_MODE; }
    Servos[servo-1].setMode(mode);
    writeServoModeToEEPROM(servo-1);
    Serial.println("Servo: " + String(servo) + " Mode: " + modeval);
    ...
    Serial.println(F("M : <servo> <SERVO|ONOFF> Set Servo Mode and store to EEPROM"));
